...
#ifdef MSDOS
# define DIRSIZ 64
# define DIRSIZE(entry)	strlen((FileInfo).achName)
...
unsigned int fmask = R_ONLY | SYSTEM | HIDDEN | SUBDIR;
int
scandir(dir, nmptr, qualify, sorter)
char	*dir;
char	***nmptr;
...
int	(*sorter) proto((UnivConstPtr, UnivConstPtr));
{
	...
	const USHORT attr = R_ONLY | SYSTEM | HIDDEN | SUBDIR;
...
#  else
	if (DosFindFirst (dirname, &DirHandle, attr, &FileInfo,
		       sizeof (FILEFINDBUF), &searchcount, 0L))
#endif
	   return (-1);
	if ((ourarray = (char **) malloc(nalloc * sizeof (char *))) == 0)
memfail:	complain("[Malloc failed: cannot scandir]");
	do  {
		if ((fmask == 0x10) && !(FileInfo.attrFile&fmask))
			goto skip;
		strlwr(FileInfo.achName);
		if (qualify != (int (*)())0 && (*qualify)(FileInfo.achName) == 0)
			goto skip;
		if (nentries == nalloc) {
			ourarray = (char **) realloc((char *) ourarray, (nalloc += 10) * sizeof (char *));
			if (ourarray == 0)
		...
		ourarray[nentries] = (char *) malloc(DIRSIZE(FileInfo) + 1);
		null_ncpy(ourarray[nentries], FileInfo.achName, (int) DIRSIZE(FileInfo));
		nentries++;
skip:	;
    }
#ifndef OS2
...
#else
    while (DosFindNext (DirHandle, &FileInfo, sizeof (FILEFINDBUF),
	       &searchcount) == 0) ;
#endif
	if ((nentries + 1) != nalloc)
		ourarray = (char **) realloc((char *) ourarray,
