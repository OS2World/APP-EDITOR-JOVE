...
getTERM()
{
	char	*getenv(EGA), *tgetstr() ;
	char	*termname;
	void	init_43(), init_50(), _fastcall init_term();
	enum	TERMTYPE termtype;
	unsigned char _fastcall LinePerPage (void);
	unsigned char _fastcall CharPerLine (void);
/* find if term is EGA */
	if (getenv("EGA") ||
	    ((getenv("TERM") != NULL) && !(stricmp(getenv("TERM"), "EGA"))))
	    termtype = ega;
	if (getenv("VGA") ||
	    ((getenv("TERM") != NULL) && !(stricmp(getenv("TERM"), "VGA"))))
	    termtype = vga;
	switch (termtype) {
	    case (ega):
	       termname = "ega";
