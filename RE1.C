...
private int substitute(re_blk, query, l1, char1, l2, char2)
struct RE_block *re_blk;
Line *l1, *l2;
int query, char1, char2;
{
    int _fastcall jgetchar(void);
    Line *lp;
    int numdone = 0, UNDO_nd = 0, offset = char1, stop = NO;
    daddr UNDO_da = 0;
    Line *UNDO_lp = NULL;
...
private void replace(query, inreg)
int query, inreg;
{
    Mark *m;
    char *rep_ptr;
    Line *l1 = curline, *l2 = curbuf->b_last;
    int char1 = curchar, char2 = length(curbuf->b_last), numdone;
    struct RE_block re_blk;
...
private int lookup(searchbuf, filebuf, tag, file)
char *searchbuf, *filebuf, *tag, *file;
{
    /* register */ size_t taglen = strlen(tag);
    char line[JBUFSIZ], pattern[128];
    /* register */ File *fp;
    struct stat stbuf;
    int success = NO;
...
void find_tag(tag, localp)
char *tag;
int localp;
{
    char filebuf[FILESIZE], sstr[100], tfbuf[FILESIZE];
    /* register */ Bufpos *bp;
    /* register */ Buffer *b;
    char *tagfname;
