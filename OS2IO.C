...
tmpinit()
{
	...
	tmp_create_fd = creat(tfname, S_IREAD | S_IWRITE);
	(void) close (tmp_create_fd);
	//(void) close(creat(tfname, S_IREAD | S_IWRITE));
#ifndef MSDOS
	tmpfd = open(tfname, 2);
#else /* MSDOS */
	//tmpfd = open(tfname, 0x8002);	/* MSDOS fix	*/
	tmpfd = open(tfname, O_RDWR);
#endif /* MSDOS */
	if (tmpfd == -1)
...
char *so, *dest;
{
	char cwd[FILESIZE], cwdD[3], cwdDIR[FILESIZE], cwdF[9], cwdEXT[5],
	     soD[_MAX_DRIVE], soDIR[_MAX_DIR], soF[_MAX_FNAME], soEXT[_MAX_EXT];
	char *drive, *path;
